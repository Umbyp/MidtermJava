...
public class ProductService {
    ...
    public List<Product> getProductByExample() {
        ...
        ExampleMatcher matcher = ExampleMatcher.matchingAll().withMatcher("productName",ignoreCase().contains())
        ...
        return productList;}
    public List<Product> getProductByCategory(String catagory){
//        return repository.findByProductLineStartingWith(catagory);
        return repository.getProductsByCategory(catagory+"%");
    }
    public List<Product> getProducts(String partOfProduct,Double lower,Double upper,String[] sortBy,String[] direction ){
        if(lower<= 0 && upper<=0){
            upper = repository.findFirstByOrderByPriceDesc().getBuyPrice();
        }
        if(lower> upper){
            double tmp = lower;
            lower = upper;
...
//        if(sortBy!= null && !sortBy.isEmpty()){
        if(sortBy!= null && sortBy.length>0){
            List<Sort.Order> sortOfserList = new ArrayList<>();
            for(int i =0 ; i< sortBy.length;i++){
                sortOfserList.add(new Sort.Order(Sort.Direction.valueOf(direction[i].toUpperCase()),sortBy[i]));
            }
//            sortOrder = new Sort.Order(Sort.Direction.valueOf(direction),sortBy);
            return repository.findByPriceBetweenAndProductNameContains(lower,upper,partOfProduct,Sort.by(sortOfserList));
        }else {
...
//        if(sortBy!= null && !sortBy.isEmpty()){
        if(sortBy!= null && sortBy.length>0){
            ...
            Pageable pageable = PageRequest.of(pageNo,pageSize,Sort.by(sortOfserList));
